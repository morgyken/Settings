...
{
    $value = InsuranceSchemePricing::whereProductId($id)
...
?>
@extends('layouts.app')
@section('content_title','Scheme Product Price')
@section('content_description','Manage Product Price for Scheme')
...
@section('content')
                            ...
                            <table class="table table-striped" id="datatable">
                                ...
                                @foreach($products as $m)
                                    <tr>
                                        ...
                                        <td>{{$m->desc}}</td>
                                        <td style="text-align: right">{{number_format($m->insurance_p,2)}}
                                        </td>
                                        <td style="text-align: right">
                                            <input type="text" name="insurance{{$m->id}}"
                                                   pid="{{$m->id}}"
                                                   value="{{modified_price($m->id,$scheme->id,$m->insurance_p)}}"/>
                                        </td>
                                        <td>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#save').click(function () {
                $.ajax({
                    url: "{{route('api.settings.pne.price.edit','product_id')}}",
                    method: 'POST',
                    data: {
                        _token: "{{csrf_token()}}",
                        data: data
